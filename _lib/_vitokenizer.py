...
import pyodbc
from _lib.ViSQL import *

class ViTokenizer:
    bi_grams = set()
    tri_grams = set()
    model_file = 'models/model.crf.pkl'
    ...
    cursor.execute("SELECT * FROM datVietWord WHERE WordCount > 1")
    for row in cursor.fetchall():
    	if row.WordCount == 2:
    	    bi_grams.add(row.Words)
    	elif row.WordCount == 3:
    	    tri_grams.add(row.Words)
                
    cursor.execute("SELECT * FROM datVietLocation WHERE WordCount > 1")
    for row in cursor.fetchall():
    	if row.WordCount == 2:
    	    bi_grams.add(row.Words)
    	elif row.WordCount == 3:
    ...
    @staticmethod
    def sqltokenize(str):
        tmp = ViSQL.sql2sylabelize(str)
        if len(tmp) == 0:
            return str
        labels = ViTokenizer.model.predict(ViSQL.sql2feature(str))
        print(ViSQL.sql2feature(str))
        print(labels)
        output = tmp[0]
        for i in range(1, len(labels[0])):
            if labels[0][i] == 'I_W' and tmp[i] not in string.punctuation and\
